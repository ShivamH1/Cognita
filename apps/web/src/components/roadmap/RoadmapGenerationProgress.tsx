...
export function RoadmapGenerationProgress() {
  const { status, progress, message, events } = useRoadmapGenerationStore()
    ...
    <div className="glass-strong rounded-3xl border border-[var(--border-md)] shadow-[var(--shadow-lg)] p-8 space-y-6 relative overflow-hidden max-w-xl mx-auto">
      ...
      <div className="text-center space-y-4">
        ...
        <div className="space-y-1">
          <h2 className="font-display text-xl font-bold text-[var(--text)] tracking-wide uppercase">
            {status === "generating" ? "AI Building Learning Roadmap" :
             status === "complete" ? "Roadmap Ready!" :
             status === "failed" ? "Roadmap Generation Failed" : "Connecting to Queue…"}
          </h2>
      ...
      <div className="border border-[var(--border)] bg-[var(--surface)] rounded-2xl p-4 h-48 overflow-y-auto space-y-2.5 shadow-inner">
        {events.length === 0 ? (
          <div className="h-full flex items-center justify-center text-xs text-[var(--text-3)] italic">
            Waiting for roadmap generation logs…
          </div>
        ) : (
          events.map((event, i) => (
      ...
      {status === "failed" && (
        <div className="text-center pt-2">
          <Button onClick={() => router.push("/roadmaps/create")} variant="outline" className="inline-flex items-center gap-2">
            <ArrowLeft className="w-4 h-4" />
            Back to Create Roadmap
          </Button>
        </div>
      )}
    </div>
